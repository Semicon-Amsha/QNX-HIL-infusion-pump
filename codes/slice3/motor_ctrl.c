...
static int process_uart_request(int fd, const pump_msg_t *msg, pump_reply_t *reply, int *alarm_coid)
{
    ...
    while (total_read < (PACKET_MAX_SIZE * 4)) {
        ...
        if (bytes_read == 0) {
            ...
            notify_alarm(alarm_coid, PULSE_FAULT_UART_TIMEOUT, msg->cmd);
        ...
        case PARSE_CRC_ERROR:
            ...
            notify_alarm(alarm_coid, PULSE_FAULT_PROTOCOL_ERROR, msg->cmd);
        ...
        case PARSE_COMPLETE:
            ...
            if (parser.cmd == CMD_FAULT) {
                ...
                notify_alarm(alarm_coid, PULSE_FAULT_REMOTE_FAULT, msg->cmd);
            ...
            if (!response_matches_request(msg->cmd, &parser)) {
                printf("Unexpected response cmd=0x%02X for request 0x%02X\n",
                       parser.cmd, msg->cmd);
                notify_alarm(alarm_coid, PULSE_FAULT_PROTOCOL_ERROR, parser.cmd);
                return PUMP_STATUS_PROTOCOL_ERROR;
            }
...
int main(void)
{
    ...
    for (;;) {
        ...
        reply.status = process_uart_request(fd, &recv.msg, &reply, &alarm_coid);
        MsgReply(rcvid, EOK, &reply, sizeof(reply));
    }
